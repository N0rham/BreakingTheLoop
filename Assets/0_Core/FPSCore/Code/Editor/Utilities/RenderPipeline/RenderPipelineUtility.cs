...
namespace PolymindGames.Editor
{
    ...
    public static class RenderPipelineUtility
    {
        ...
        public static RenderPipelineType GetRenderingPipeline()
        {
            ...
            string pipelineTypeName = pipelineAsset.GetType().FullName ?? string.Empty;
            if (pipelineTypeName.Contains("HighDefinition", StringComparison.Ordinal))
                return RenderPipelineType.HDRP;

            if (pipelineTypeName.Contains("Universal", StringComparison.Ordinal))
        ...
        public static void ReconvertMaterialsForCurrentPipeline()
        {
            var pipelineType = GetRenderingPipeline();
            RenderPipelineMaterialUtility.ConvertAllMaterialsAtPath(pipelineType);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            Debug.Log($"Reconverted Polymind materials for {pipelineType}.");
        }
        ...
        private static void SetRenderingPipeline(RenderPipelineType fromPipeline, RenderPipelineType targetPipeline, int conversionStep = 0)
        {
            ...
            static IEnumerator Part2(RenderPipelineType fromPipeline, RenderPipelineType targetPipeline)
            {
                ...
                string symbolToAdd = GetDefineSymbolForPipeline(targetPipeline);
                if (symbolToAdd != null)
