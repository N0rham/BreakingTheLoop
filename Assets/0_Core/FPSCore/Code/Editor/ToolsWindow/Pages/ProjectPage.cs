...
namespace PolymindGames.Editor
{
    ...
    public sealed class ProjectPage : RootToolPage
    {
        ...
        private static void DrawRenderPipelines()
        {
            ...
            {
                EditorGUILayout.HelpBox("If some Polymind materials are still on the Built-in pipeline after a failed or partial conversion, rerun the material converter for the active pipeline.", MessageType.None);

                if (GUILayout.Button("Reconvert Materials", GUILayout.Width(300f)))
                {
                    var activePipeline = RenderPipelineUtility.GetCleanPipelineName(RenderPipelineUtility.GetRenderingPipeline());
                    if (EditorUtility.DisplayDialog("Reconvert Materials",
                            $"Rerun the material conversion for {activePipeline}? This only affects materials under Assets/PolymindGames.",
                            "Ok", "Cancel"))
                    {
                        RenderPipelineUtility.ReconvertMaterialsForCurrentPipeline();
                    }
                }
            }
